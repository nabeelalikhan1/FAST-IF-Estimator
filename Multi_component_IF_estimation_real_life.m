...
addpath('D:\tfsa_5-5\windows\win64_bin');
 Sig=Sig;%+2*0.04*cos(2*pi*(0.01*n+0.000007*n.^3)); 
...
Sig=decimate(Sig,2);
addpath('D:\tfsa_5-5\windows\win64_bin');
 Sig=Sig+1*0.04*cos(2*pi*(0.01*n+0.000007*n.^3))+1*0.04*cos(2*pi*(0.48*n-0.000009*n.^3)); 
...
% Sig=hilbert(Sig);
% tic
% [fidexmult,A] = non_tfd_IF_new_display(Sig,length(Sig)/(2)-1, num+10, 2,30,0.02,0);
% toc
% t=0:1/SampFreq:8-1/SampFreq;
% TF=zeros(length(Sig),length(Sig));
% [N,M]=size(fidexmult);
