...
function [fidexmult] = FAST_IF_c(Sig,win_length, num, delta,L,thr,Thr)
...
for iii=1:num
    ...
    if freq_start<length(Sig)/2
        IF(t_start)=freq_start;
    else
        IF(t_start)=-length(Sig)+freq_start;
    end
